...
#     do
# 	#	    echo $LINE
# 	wget $LINE -nv -O - >> /tmp/deadduck & #without this backgroudn we only get one image.
# 	sleep 0.20     # slow the http requests down a bit so we don't get blocked from the server
#     done < ${1}
# }
...
#     wget http://gingerhaze.com/deadduck/archive -O /tmp/deadduckarchive
#     lastpage=$(grep "Go to last page" /tmp/deadduckarchive | sed 's/.*page=//;s/".*//')

#     echo "http://gingerhaze.com/deadduck/archive" > /tmp/deadduckarchive

#     echo "lastpage ${lastpage}"
#     counter=${lastpage}
#     while [ $counter -gt 0 ]; do
# 	echo "http://gingerhaze.com/deadduck/archive?page=${counter}/ "  >> /tmp/deadduckarchive
# 	let "counter-=1"
#     done
#     performWgetOnFileGeneric /tmp/deadduckarchive /tmp/deadducksinglepagelinks
...
#     grep '<a href="/deadduck/comic/' /tmp/deadducksinglepagelinks | sed 's/.*a href="\/deadduck\/comic\//http:\/\/www.gingerhaze.com\/deadduck\/comic\//;s/".*//' >> /tmp/deadducksinglepagelinksabsolute
#     performWgetOnFileGeneric  /tmp/deadducksinglepagelinksabsolute  /tmp/deadduckallinone
# }

# function getIndividualImages()  {
#     grep foaf:Image /tmp/deadduckallinone | sed 's/.*foaf:Image" src="//;s/".*//' >> /tmp/deadduckimages
#     SortUniqFile /tmp/deadduckimages 
#     performWgetOnImages /tmp/deadduckimages
# }
# function getHTMLPages () {
#     echo "getHTMLPages"
#     counter=${1}
#     while [ $counter -gt 0 ]; do
# 	echo "http://deadduck.enenkay.com/comic/${counter}/ " >> /tmp/deadduckpages
# 	#echo ${counter}
# 	let "counter-=1"
#     done

#     performWgetOnFile /tmp/deadduckpages
# }
...
#     cat $1 | xargs -P $numberOfParallelOperations -r -n 1 wget -nv -O - >> /tmp/deadduck
# }
...
function getImageFromPage()  {
    ...
    echo "${image}"
    echo "${image}" >> /tmp/deadduckImages.txt
    echo "deadduck_${fileNumber}.$fileExtension"
    wget ${image} -nv -nc -U "IE9" -O deadduck_${fileNumber}.$fileExtension &
...
echo ${nextURL} >> /tmp/deadducklog.txt
echo "--${nextURL}--"
if [ "${nextURL}" != "" ]
then
    wget "${nextURL}" -O /tmp/deadducksinglepage
fi
}
...
then 
	...
	wget http://www.jayfosgitt.com/deadduckcomic/?p=104 -U "Mozilla" -O /tmp/deadducksinglepage
	oldNextUrl="blank"
	nextUrl="empty"
	while [ "${nextUrl}" != "${oldNextUrl}" ] && [ "${nextUrl}" != "" ]  # you need the quotes for it to work
	do
	    oldNextUrl=${nextUrl}	    
	    getImageFromPage /tmp/deadducksinglepage
	    nextUrl=$( getNextUrlFromPage /tmp/deadducksinglepage )
	...
	zip Deadduck deadduck*.${fileExtension}
	mv Deadduck.zip Deadduck.cbz
	evince Deadduck.cbz  &
    fi 
fi
echo ""
