...
#http://blindsprings.com/wp-content/uploads/2014/03/TB_02_42.jpg
...
function performWgetOnFile () {
    ...
    do
	#	    echo $LINE
	wget $LINE -nv -U "IE9" -O - >> /tmp/blindsprings & #without this backgroudn we only get one image.
	sleep 0.20     # slow the http requests down a bit so we don't get blocked from the server
    done < ${1}
}
...
function getHTMLPages () {
    ...
    while [ $counter -ge 0 ]; do
    	...
    	echo "http://blindsprings.com/comic/${fileName}/ " >> /tmp/blindspringspages
    	#echo ${counter}
    	let "counter-=1"
    done

#    performWgetOnFileForImages /tmp/blindspringspages
    performWgetOnFile /tmp/blindspringspages
}
...
function getPagesInFile() {
    ...
    cat $1 | xargs -P $numberOfParallelOperations -r -n 1 wget -nv -O - >> /tmp/blindsprings
}
...
#<img src="http://blindsprings.com/wp-content/uploads/2014/03/TB_02_42.jpg" alt="Ah
	grep "blindsprings.com/wp-content/uploads" /tmp/blindsprings | sed 's/.*img src="//;s/".*//;' | sort | uniq  > /tmp/blindspringscomicimages


	performWgetOnFileForImages /tmp/blindspringscomicimages
