...
#     do
# 	#	    echo $LINE
# 	wget $LINE -nv -O - >> /tmp/monsterisle & #without this backgroudn we only get one image.
# 	sleep 0.20     # slow the http requests down a bit so we don't get blocked from the server
#     done < ${1}
# }
...
#     wget http://gingerhaze.com/monsterisle/archive -O /tmp/monsterislearchive
#     lastpage=$(grep "Go to last page" /tmp/monsterislearchive | sed 's/.*page=//;s/".*//')

#     echo "http://gingerhaze.com/monsterisle/archive" > /tmp/monsterislearchive

#     echo "lastpage ${lastpage}"
#     counter=${lastpage}
#     while [ $counter -gt 0 ]; do
# 	echo "http://gingerhaze.com/monsterisle/archive?page=${counter}/ "  >> /tmp/monsterislearchive
# 	let "counter-=1"
#     done
#     performWgetOnFileGeneric /tmp/monsterislearchive /tmp/monsterislesinglepagelinks
...
#     grep '<a href="/monsterisle/comic/' /tmp/monsterislesinglepagelinks | sed 's/.*a href="\/monsterisle\/comic\//http:\/\/www.gingerhaze.com\/monsterisle\/comic\//;s/".*//' >> /tmp/monsterislesinglepagelinksabsolute
#     performWgetOnFileGeneric  /tmp/monsterislesinglepagelinksabsolute  /tmp/monsterisleallinone
# }

# function getIndividualImages()  {
#     grep foaf:Image /tmp/monsterisleallinone | sed 's/.*foaf:Image" src="//;s/".*//' >> /tmp/monsterisleimages
#     SortUniqFile /tmp/monsterisleimages 
#     performWgetOnImages /tmp/monsterisleimages
# }
# function getHTMLPages () {
#     echo "getHTMLPages"
#     counter=${1}
#     while [ $counter -gt 0 ]; do
# 	echo "http://monsterisle.enenkay.com/comic/${counter}/ " >> /tmp/monsterislepages
# 	#echo ${counter}
# 	let "counter-=1"
#     done

#     performWgetOnFile /tmp/monsterislepages
# }
...
#     cat $1 | xargs -P $numberOfParallelOperations -r -n 1 wget -nv -O - >> /tmp/monsterisle
# }

function getImageFromPage()  {
    image=$(  grep 'id="comic"'  -A 1 ${1} | grep img | sed 's/.*img src="//;s/".*//' )
    fileNumber=$( printf "%03d" ${imagefilecounter} )
    echo "${image}"
    echo "${image}" >> /tmp/monsterisleImages.txt
    echo "monsterisle_${fileNumber}.$fileExtension"
    wget ${image} -nv -nc -U "IE9" -O monsterisle_${fileNumber}.$fileExtension &
...
echo ${nextURL} >> /tmp/monsterislelog.txt
echo "--${nextURL}--"
if [ "${nextURL}" != "" ]
then
    wget "${nextURL}" -O /tmp/monsterislesinglepage
fi
}
...
then 
	...
	wget http://tragic-planet.com/monsterisle/?p=17 -U "Mozilla" -O /tmp/monsterislesinglepage
	oldNextUrl="blank"
	nextUrl="empty"
	while [ "${nextUrl}" != "${oldNextUrl}" ] && [ "${nextUrl}" != "" ]  # you need the quotes for it to work
	do
	    oldNextUrl=${nextUrl}	    
	    getImageFromPage /tmp/monsterislesinglepage
	    nextUrl=$( getNextUrlFromPage /tmp/monsterislesinglepage )
	...
	zip Monsterisle monsterisle*.${fileExtension}
	mv Monsterisle.zip Monsterisle.cbz
	evince Monsterisle.cbz  &
    fi 
fi
echo ""
