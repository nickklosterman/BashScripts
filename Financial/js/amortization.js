...
//If the month = January, this function will output the hypothetical new taxes owed for the year based on the appreciation rate, otherwise it'll output the monthly taxes owed
function computeTaxIncrease(idx,taxes) {
    var yearlyTaxes =  document.getElementById("taxes");
    var appreciationRate =  document.getElementById("appreciationRate");
    
    if (false) {
	var base = Math.pow(1+parseFloat(appreciationRate.value)/100, 1/12) ;

	var app = Math.pow(base,idx);//1+  parseInt(idx/12)/100;


	var yearlyTaxes = (taxes*app);
	var monthlyTaxes = yearlyTaxes / 12;
	console.log(idx,base,yearlyTaxes.toFixed(2),monthlyTaxes.toFixed(2));
    }
    else {
	var   app = Math.pow(1+parseFloat(appreciationRate.value)/100, parseInt(idx/12));
	var yearlyTaxes = (taxes*app);
	var monthlyTaxes = yearlyTaxes / 12;
	//console.log(idx,base,yearlyTaxes.toFixed(2),monthlyTaxes.toFixed(2));

	return monthlyTaxes.toFixed(2);
    }

}

function payPeriodDate(idx,currentDate){
    var months= ["January","February","March","April","May","June","July","August","September","October","November","December"];
    var year = currentDate.getYear() + 1900 + Math.floor((idx+currentDate.getMonth()) / 12);
    var month = months[(currentDate.getMonth()  + idx) % 12];
    var dateString = month + " " + year;
    return dateString; //{ month: month, year: year };	
} ; 
...
function calculate() {
    ...
    var yearlyTaxes =  document.getElementById("taxes");
    var monthlyTaxes = parseFloat(yearlyTaxes.value) / 12;
    var appreciationRate =  document.getElementById("appreciationRate");
	...
	taxpayment.innerHTML = monthlyTaxes.toFixed(2);
