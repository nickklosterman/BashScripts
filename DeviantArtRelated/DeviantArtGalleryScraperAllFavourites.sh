...
then 
	...
	offsetcounter=0
new_directory=${1}-favourites
#create a directory with the deviantID and move into it for saving of images
	echo "Making directory ${new_directory} and moving into it"
	mkdir "$new_directory"
	cd "$new_directory"
	
#get favourites index
	echo "Getting favourites index file"
#without the trailing / it saves the file as favourites instead of index.html
	wget -U Mozilla   http://${1}.deviantart.com/favourites/ #-O $OutputFile
#check if the favourites index has additional pages.
...
#echo "$NextPageCheck"	
#grab all pages of the favourites
	while [ "$NextPageCheck" != "" ]
	do 
	    
	    let 'offsetcounter+=1'
	    let "offset=${offsetcounter} * 24"
	    echo "Getting favourites index with offset of ${offset}"
	...
	echo "Done getting all favourites pages."
...
#Since the full images are first priority, we now try to get the fullimg then img listed in the favourites index file (as they are of lower res)	
