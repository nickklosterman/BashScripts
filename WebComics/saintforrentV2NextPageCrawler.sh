...
function performWgetOnFile () {
    ...
    do
	#	    echo $LINE
	wget $LINE -nv -O - >> /tmp/saintforrent & #without this backgroudn we only get one image.
	sleep 0.20     # slow the http requests down a bit so we don't get blocked from the server
    done < ${1}
}
...
function getArchivePages() {
    ...
    wget http://gingerhaze.com/saintforrent/archive -O /tmp/saintforrentarchive
    lastpage=$(grep "Go to last page" /tmp/saintforrentarchive | sed 's/.*page=//;s/".*//')

    echo "http://gingerhaze.com/saintforrent/archive" > /tmp/saintforrentarchive

    echo "lastpage ${lastpage}"
    counter=${lastpage}
    while [ $counter -gt 0 ]; do
	echo "http://gingerhaze.com/saintforrent/archive?page=${counter}/ "  >> /tmp/saintforrentarchive
	let "counter-=1"
    done
    performWgetOnFileGeneric /tmp/saintforrentarchive /tmp/saintforrentsinglepagelinks
...
function getIndividualPages() {
    grep '<a href="/saintforrent/comic/' /tmp/saintforrentsinglepagelinks | sed 's/.*a href="\/saintforrent\/comic\//http:\/\/www.gingerhaze.com\/saintforrent\/comic\//;s/".*//' >> /tmp/saintforrentsinglepagelinksabsolute
#    grep 'foaf:Image' /tmp/saintforrentsinglepagelinks | sed 's/.*a href="\/saintforrent\/comic\//http:\/\/www.gingerhaze.com\/saintforrent\/comic\//;s/".*//' >> /tmp/saintforrentsinglepagelinksabsolute

    performWgetOnFileGeneric  /tmp/saintforrentsinglepagelinksabsolute  /tmp/saintforrentallinone
}

function getIndividualImages()  {
#    grep foaf:Image /tmp/saintforrentallinone | sed 's/.*src="//;s/".*//' >> /tmp/saintforrentimages
    grep foaf:Image /tmp/saintforrentallinone | sed 's/.*foaf:Image" src="//;s/".*//' >> /tmp/saintforrentimages
    SortUniqFile /tmp/saintforrentimages 

    performWgetOnImages /tmp/saintforrentimages
}

function getImageFromPage()  {
    image=$(  grep 'id="comic"' ${1} | sed 's/.*img src="comics\/../http:\/\/www.saintforrent.com/;s/".*//' )
    fileNumber=$( printf "%03d" ${imagefilecounter} )
    echo "${image}"
    echo "${image}" >> /tmp/saintforrentImages.txt
    echo "saintforrent_${fileNumber}.$fileExtension"
    wget ${image} -nv -nc -U "IE9" -O saintforrent_${fileNumber}.$fileExtension &
...
function getHTMLPages () {
    ...
    counter=${1}
    while [ $counter -gt 0 ]; do
	echo "http://saintforrent.enenkay.com/comic/${counter}/ " >> /tmp/saintforrentpages
	#echo ${counter}
	let "counter-=1"
    done
...
#Step 2 here:
nextURL=$( grep 'class="next"' "${1}" | uniq | sed 's/.*href="\/index.php/http:\/\/www.saintforrent.com/;s/".*//' )
echo ${nextURL} >> /tmp/saintforrentlog.txt
echo "--${nextURL}--"
if [ "${nextURL}" != "" ]
then
    wget "${nextURL}" -O /tmp/saintforrentsinglepage
fi
}
...
then 
	...
	wget http://www.saintforrent.com/index.php?id=1 -U "Mozilla" -O /tmp/saintforrentsinglepage
	oldNextUrl="blank"
	nextUrl="empty"
	while [ "${nextUrl}" != "${oldNextUrl}" ] && [ "${nextUrl}" != "" ]  # you need the quotes for it to work
	do
	    oldNextUrl=${nextUrl}	    
	    getImageFromPage /tmp/saintforrentsinglepage
	    nextUrl=$( getNextUrlFromPage /tmp/saintforrentsinglepage )
	...
	zip Saintforrent saintforrent*.${fileExtension}
	mv Saintforrent.zip Saintforrent.cbz
	evince Saintforrent.cbz  &
    fi 
fi
echo "Saint For Rent uses animated gifs and evince can't display those :("
