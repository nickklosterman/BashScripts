...
#     do
# 	#	    echo $LINE
# 	wget $LINE -nv -O - >> /tmp/cleopatrainspace & #without this backgroudn we only get one image.
# 	sleep 0.20     # slow the http requests down a bit so we don't get blocked from the server
#     done < ${1}
# }
...
#     wget http://gingerhaze.com/cleopatrainspace/archive -O /tmp/cleopatrainspacearchive
#     lastpage=$(grep "Go to last page" /tmp/cleopatrainspacearchive | sed 's/.*page=//;s/".*//')

#     echo "http://gingerhaze.com/cleopatrainspace/archive" > /tmp/cleopatrainspacearchive

#     echo "lastpage ${lastpage}"
#     counter=${lastpage}
#     while [ $counter -gt 0 ]; do
# 	echo "http://gingerhaze.com/cleopatrainspace/archive?page=${counter}/ "  >> /tmp/cleopatrainspacearchive
# 	let "counter-=1"
#     done
#     performWgetOnFileGeneric /tmp/cleopatrainspacearchive /tmp/cleopatrainspacesinglepagelinks
...
#     grep '<a href="/cleopatrainspace/comic/' /tmp/cleopatrainspacesinglepagelinks | sed 's/.*a href="\/cleopatrainspace\/comic\//http:\/\/www.gingerhaze.com\/cleopatrainspace\/comic\//;s/".*//' >> /tmp/cleopatrainspacesinglepagelinksabsolute
#     performWgetOnFileGeneric  /tmp/cleopatrainspacesinglepagelinksabsolute  /tmp/cleopatrainspaceallinone
# }

# function getIndividualImages()  {
#     grep foaf:Image /tmp/cleopatrainspaceallinone | sed 's/.*foaf:Image" src="//;s/".*//' >> /tmp/cleopatrainspaceimages
#     SortUniqFile /tmp/cleopatrainspaceimages 
#     performWgetOnImages /tmp/cleopatrainspaceimages
# }
# function getHTMLPages () {
#     echo "getHTMLPages"
#     counter=${1}
#     while [ $counter -gt 0 ]; do
# 	echo "http://cleopatrainspace.enenkay.com/comic/${counter}/ " >> /tmp/cleopatrainspacepages
# 	#echo ${counter}
# 	let "counter-=1"
#     done

#     performWgetOnFile /tmp/cleopatrainspacepages
# }
...
#     cat $1 | xargs -P $numberOfParallelOperations -r -n 1 wget -nv -O - >> /tmp/cleopatrainspace
# }
...
function getImageFromPage()  {
    ...
    echo "${image}"
    echo "${image}" >> /tmp/cleopatrainspaceImages.txt
    echo "cleopatrainspace_${fileNumber}.$fileExtension"
    wget ${image} -nv -nc -U "IE9" -O cleopatrainspace_${fileNumber}.$fileExtension &
...
echo ${nextURL} >> /tmp/cleopatrainspacelog.txt
echo "--${nextURL}--"
if [ "${nextURL}" != "" ]
then
    wget "${nextURL}" -O /tmp/cleopatrainspacesinglepage
fi
}
...
then 
	...
	wget http://www.cowshell.com/cleopatra/2009/08/17/chap-1-pg-1/ -U "Mozilla" -O /tmp/cleopatrainspacesinglepage
	oldNextUrl="blank"
	nextUrl="empty"
	while [ "${nextUrl}" != "${oldNextUrl}" ] && [ "${nextUrl}" != "" ]  # you need the quotes for it to work
	do
	    oldNextUrl=${nextUrl}	    
	    getImageFromPage /tmp/cleopatrainspacesinglepage
	    nextUrl=$( getNextUrlFromPage /tmp/cleopatrainspacesinglepage )
	...
	zip Cleopatrainspace cleopatrainspace*.${fileExtension}
	mv Cleopatrainspace.zip Cleopatrainspace.cbz
	evince Cleopatrainspace.cbz  &
    fi 
fi
echo ""
