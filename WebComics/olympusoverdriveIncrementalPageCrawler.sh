...
function getImageFromPage()  {
    ...
    if [ "${image}" != "http://www.olympusoverdrive.com/comics/" ]
    then 
	if [ "${image}" = "	<div id=" ]
	then
	    convert -background white -fill black -pointsize 24 label:"no image${fileNumber}"  olympusoverdrive_${fileNumber}.${fileExtension}
	    echo "${fileNumber}" >> /tmp/olympusoverdriveImages.txt
	else
	    echo "${image}"
...
wget http://www.olympusoverdrive.com/index.php -O /tmp/olympusoverdrivesinglepage
lastpage=$( grep 'class="last"' /tmp/olympusoverdrivesinglepage | sed 's/.*rel="next"//;s/.*id=//;s/".*//' )
echo ${lastpage}
}
...
#	echo "${lastpage}"
	...
	do
	    wget http://www.olympusoverdrive.com/index.php?id=${counter} -U "Mozilla" -O /tmp/olympusoverdrivesinglepage
	...
	rm olympusoverdrive*.${fileExtension}
    fi 
fi
echo ""
