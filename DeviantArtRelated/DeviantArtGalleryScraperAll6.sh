...
function downloadMatureImageFromURL ()
{
	...
	# grep 'gmi-ResourceViewShareTumblr' "${tempPage}" | sed 's/.*openurl="http:\/\/www.tumblr.com\/share\/photo?source=//;s/&amp;caption=.*//;s/%2F/\//;s/%3A/:/'  )
...
then 
	...
	echo "${MatureLinks}" > MatureContent.txt
	...
	then 
	    ...
	    echo "--------------mature=--------------"
	    echo "--------------mature=--------------"
	    downloadMatureImageFromURL "${MatureLinks}"
	else 
	    echo "**********************"
	fi
