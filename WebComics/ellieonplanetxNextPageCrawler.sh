...
#     do
# 	#	    echo $LINE
# 	wget $LINE -nv -O - >> /tmp/ellieonplanetx & #without this backgroudn we only get one image.
# 	sleep 0.20     # slow the http requests down a bit so we don't get blocked from the server
#     done < ${1}
# }
...
#     wget http://gingerhaze.com/ellieonplanetx/archive -O /tmp/ellieonplanetxarchive
#     lastpage=$(grep "Go to last page" /tmp/ellieonplanetxarchive | sed 's/.*page=//;s/".*//')

#     echo "http://gingerhaze.com/ellieonplanetx/archive" > /tmp/ellieonplanetxarchive

#     echo "lastpage ${lastpage}"
#     counter=${lastpage}
#     while [ $counter -gt 0 ]; do
# 	echo "http://gingerhaze.com/ellieonplanetx/archive?page=${counter}/ "  >> /tmp/ellieonplanetxarchive
# 	let "counter-=1"
#     done
#     performWgetOnFileGeneric /tmp/ellieonplanetxarchive /tmp/ellieonplanetxsinglepagelinks
...
#     grep '<a href="/ellieonplanetx/comic/' /tmp/ellieonplanetxsinglepagelinks | sed 's/.*a href="\/ellieonplanetx\/comic\//http:\/\/www.gingerhaze.com\/ellieonplanetx\/comic\//;s/".*//' >> /tmp/ellieonplanetxsinglepagelinksabsolute
#     performWgetOnFileGeneric  /tmp/ellieonplanetxsinglepagelinksabsolute  /tmp/ellieonplanetxallinone
# }

# function getIndividualImages()  {
#     grep foaf:Image /tmp/ellieonplanetxallinone | sed 's/.*foaf:Image" src="//;s/".*//' >> /tmp/ellieonplanetximages
#     SortUniqFile /tmp/ellieonplanetximages 
#     performWgetOnImages /tmp/ellieonplanetximages
# }
# function getHTMLPages () {
#     echo "getHTMLPages"
#     counter=${1}
#     while [ $counter -gt 0 ]; do
# 	echo "http://ellieonplanetx.enenkay.com/comic/${counter}/ " >> /tmp/ellieonplanetxpages
# 	#echo ${counter}
# 	let "counter-=1"
#     done

#     performWgetOnFile /tmp/ellieonplanetxpages
# }
...
#     cat $1 | xargs -P $numberOfParallelOperations -r -n 1 wget -nv -O - >> /tmp/ellieonplanetx
# }
...
function getImageFromPage()  {
    ...
    echo "${image}"
    echo "${image}" >> /tmp/ellieonplanetxImages.txt
    echo "ellieonplanetx_${fileNumber}.$fileExtension"
    wget ${image} -nv -nc -U "IE9" -O ellieonplanetx_${fileNumber}.$fileExtension &
...
echo ${nextURL} >> /tmp/ellieonplanetxlog.txt
echo "--${nextURL}--"
if [ "${nextURL}" != "" ]
then
    wget "${nextURL}" -O /tmp/ellieonplanetxsinglepage
fi
}
...
then 
	...
	wget http://ellieonplanetx.com/comic/ellie/ -U "Mozilla" -O /tmp/ellieonplanetxsinglepage
	oldNextUrl="blank"
	nextUrl="empty"
	while [ "${nextUrl}" != "${oldNextUrl}" ] && [ "${nextUrl}" != "" ]  # you need the quotes for it to work
	do
	    oldNextUrl=${nextUrl}	    
	    getImageFromPage /tmp/ellieonplanetxsinglepage
	    nextUrl=$( getNextUrlFromPage /tmp/ellieonplanetxsinglepage )
	...
	zip Ellieonplanetx ellieonplanetx*.${fileExtension}
	mv Ellieonplanetx.zip Ellieonplanetx.cbz
	evince Ellieonplanetx.cbz  &
    fi 
fi
echo ""
