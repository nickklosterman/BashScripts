...
#wget -q "http://www.nws.noaa.gov/view/prodsByState.php?state=OH&prodtype=zone" -O /tmp/fc.html && grep -A 28 -m 1 OHZ077 /tmp/fc.html #without the -m 1 we get the latest 3 weather reports.


wget -q "http://www.nws.noaa.gov/view/prodsByState.php?state=OH&prodtype=zone" -O /tmp/fc.html && awk '/OHZ077/{found=1} found{print; if(/\$\$/) exit}' /tmp/fc.html 
