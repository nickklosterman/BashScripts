...
#     do
# 	#	    echo $LINE
# 	wget $LINE -nv -O - >> /tmp/oysterwar & #without this backgroudn we only get one image.
# 	sleep 0.20     # slow the http requests down a bit so we don't get blocked from the server
#     done < ${1}
# }
...
#     wget http://gingerhaze.com/oysterwar/archive -O /tmp/oysterwararchive
#     lastpage=$(grep "Go to last page" /tmp/oysterwararchive | sed 's/.*page=//;s/".*//')

#     echo "http://gingerhaze.com/oysterwar/archive" > /tmp/oysterwararchive

#     echo "lastpage ${lastpage}"
#     counter=${lastpage}
#     while [ $counter -gt 0 ]; do
# 	echo "http://gingerhaze.com/oysterwar/archive?page=${counter}/ "  >> /tmp/oysterwararchive
# 	let "counter-=1"
#     done
#     performWgetOnFileGeneric /tmp/oysterwararchive /tmp/oysterwarsinglepagelinks
...
#     grep '<a href="/oysterwar/comic/' /tmp/oysterwarsinglepagelinks | sed 's/.*a href="\/oysterwar\/comic\//http:\/\/www.gingerhaze.com\/oysterwar\/comic\//;s/".*//' >> /tmp/oysterwarsinglepagelinksabsolute
#     performWgetOnFileGeneric  /tmp/oysterwarsinglepagelinksabsolute  /tmp/oysterwarallinone
# }

# function getIndividualImages()  {
#     grep foaf:Image /tmp/oysterwarallinone | sed 's/.*foaf:Image" src="//;s/".*//' >> /tmp/oysterwarimages
#     SortUniqFile /tmp/oysterwarimages 
#     performWgetOnImages /tmp/oysterwarimages
# }
# function getHTMLPages () {
#     echo "getHTMLPages"
#     counter=${1}
#     while [ $counter -gt 0 ]; do
# 	echo "http://oysterwar.enenkay.com/comic/${counter}/ " >> /tmp/oysterwarpages
# 	#echo ${counter}
# 	let "counter-=1"
#     done

#     performWgetOnFile /tmp/oysterwarpages
# }
...
#     cat $1 | xargs -P $numberOfParallelOperations -r -n 1 wget -nv -O - >> /tmp/oysterwar
# }
...
function getImageFromPage()  {
    ...
    echo "${image}"
    echo "${image}" >> /tmp/oysterwarImages.txt
    echo "oysterwar_${fileNumber}.$fileExtension"
    wget "${image}" -nv -nc -U "IE9" -O oysterwar_${fileNumber}.$fileExtension &
...
echo ${nextURL} >> /tmp/oysterwarlog.txt
echo "--${nextURL}--"
if [ "${nextURL}" != "" ]
then
    wget "${nextURL}" -O /tmp/oysterwarsinglepage
fi
}
...
then 
	...
	performCleanup
	startUrl="http://oysterwar.com/?webcomic_post=page-01-2"
	#Step 1 here: this is the start page to begin scraping from
	wget "${startUrl}"  -U "Mozilla" -O /tmp/oysterwarsinglepage
	oldNextUrl="blank"
	nextUrl="empty"
	while [ "${nextUrl}" != "${oldNextUrl}" ] && [ "${nextUrl}" != "" ]  # you need the quotes for it to work
	do
	    oldNextUrl=${nextUrl}	    
	    getImageFromPage /tmp/oysterwarsinglepage
	    nextUrl=$( getNextUrlFromPage /tmp/oysterwarsinglepage )
	...
	zip Oysterwar oysterwar*.${fileExtension}
	mv Oysterwar.zip Oysterwar.cbz
	evince Oysterwar.cbz  &
	rm oysterwar*.${fileExtension}
    fi 
fi
echo ""
