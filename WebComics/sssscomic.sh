...
function performWgetOnFile () {
    ...
    do
	#	    echo $LINE
	wget $LINE -nv -O - >> /tmp/sssscomic & #without this backgroudn we only get one image.
	sleep 0.20     # slow the http requests down a bit so we don't get blocked from the server
    done < ${1}
}
...
function getHTMLPages () {
	...
	echo "http://www.sssscomic.com/comicpages/${counter}.${fileExtension} " >> /tmp/sssscomicpages
	#echo ${counter}
	let "counter-=1"
    done

    performWgetOnFileForImages /tmp/sssscomicpages
}
...
function getPagesInFile() {
    ...
    cat $1 | xargs -P $numberOfParallelOperations -r -n 1 wget -nv -O - >> /tmp/sssscomic
}
...
#	getHTMLPages $NumberOfImages
	...
	# wait for /tmp/sssscomic to be populated
	sleep 3s	
	zeroPadNumericFileNames 

	fileCheck $NumberOfImages

	zip Sssscomic *.${fileExtension}
	mv Sssscomic.zip Sssscomic.cbz
	evince Sssscomic.cbz  &
    fi 
fi
