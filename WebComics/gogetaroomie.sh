...
function performWgetOnFile () {
    ...
    do
	#	    echo $LINE
	wget $LINE -nv -U "IE9" -O - >> /tmp/gogetaroomie & #without this backgroudn we only get one image.
	sleep 0.20     # slow the http requests down a bit so we don't get blocked from the server
    done < ${1}
}
...
#    	fileName=$(printf "%03d" ${counter})
    	echo "http://www.gogetaroomie.com/index.php?id=${counter}/ " >> /tmp/gogetaroomiepages
    	#echo ${counter}
    	let "counter-=1"
    done

#    performWgetOnFileForImages /tmp/gogetaroomiepages
...
#<img src="comics/../comics/1343242330-2012-05-12-Stop-Loving-Me.jpg" id="comic" border="0"
#http://www.gogetaroomie.com/comics/1342477764-2010-07-11-go-get-a-roomie.jpg

#	grep 'id="comic"' /tmp/gogetaroomie | sed 's/.*img src="comics\//"http:\/\/www.gogetaroomie.com\/comics\//;s/".*/"/' | sort | uniq  > /tmp/gogetaroomiecomicimages

#for some reason adding a " before the http screwed stuff up so I simply did a beginning of line replacement/addition
#	grep 'id="comic"' /tmp/gogetaroomie | sed 's/.*img src="comics\//http:\/\/www.gogetaroomie.com\/comics\//;s/".*/"/;s/^/"/' | sort | uniq  > /tmp/gogetaroomiecomicimages

#I was going to add quotes to the url here, but that isn't where you want to do it. 
#you want to add quotes to the variable/line from the file you are reading in via just doing "${foo}" and that solves the names with spaces problem
	grep 'id="comic"' /tmp/gogetaroomie | sed 's/.*img src="comics\//http:\/\/www.gogetaroomie.com\/comics\//;s/".*//;' | sort | uniq  > /tmp/gogetaroomiecomicimages


	performWgetOnFileForImages /tmp/gogetaroomiecomicimages
